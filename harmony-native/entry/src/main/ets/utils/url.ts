/**
 * 知乎 URL 解析 —— 移植自上游 zhihu--/utils/url.ts。
 * 改动：new URL → @kit.ArkTS URL。
 */
...
const ZHIHU_WEB_HOSTS: Set<string> = new Set<string>([
  'zhihu.com',
  'www.zhihu.com',
  'zhuanlan.zhihu.com',
  'oia.zhihu.com',
...
function extractPath(url: string): string | null {
  ...
  const parsedUrl = new URL_CTOR(trimmedUrl);
...
function normalizeSupportedPath(path: string): string | null {
  ...
  for (const routePattern of routePatterns) {
    const match = withoutOia.match(routePattern.pattern);
    if (match) return routePattern.buildPath(match);
  }
