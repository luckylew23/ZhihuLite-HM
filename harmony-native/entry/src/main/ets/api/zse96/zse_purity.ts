/**
 * ZSE-96 纯 TS 签名（v2.0）—— 移植自上游 zhihu--/api/zse96/zse_purity.ts。
 * 唯一改动：MD5 由 expo-crypto 换成 utils/md5.ts 的纯 TS 实现。
 */
...
export const getSignaturePurity = async (
  ...
  // 1. MD5 哈希
  const md5HexResult = md5Hex(source);

  // 2. 构造明文 (Seed 固定为 12)
  const seed = 12;
  const encodedMd5 = encodeURIComponent(md5HexResult);
